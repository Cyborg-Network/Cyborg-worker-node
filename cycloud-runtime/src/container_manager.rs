use bollard::query_parameters::{
    InspectContainerOptions, InspectNetworkOptions, RemoveContainerOptions, StartContainerOptions,
...
impl<'a> ContainerManager<'a> {
    pub async fn new(
        setup_args: ContainerProvisionArgs<'a>,
        ...
        for attempt in 0..MAX_SETUP_ATTEMPTS {
            ...
            if let Some(container_info) = container {
                ...
                match state.and_then(|s| s.status.as_ref()) {
                    ...
                    _ => {
                        println!("Container {} has unexpected state, reprovisioning...", id);
                        let _ = self.remove_container().await;
                ...
                println!(
                    "Container {} not found, provisioning...",
                    self.container_name
                );
                if let Err(e) = self.provision_new_container().await {
                    println!("Failed to provision container: {}", e);
    ...
    pub async fn provision_new_container(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn provision_container(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let compose_content = Resources::DOCKER_COMPOSE
            .content
    ...
    pub async fn ensure_network(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self
            .docker
            .inspect_network("container-network", None::<InspectNetworkOptions>)
            .await
            .is_ok()
        {
            return Ok(());
    ...
    pub async fn cleanup_impl(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn stop_impl(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn restart_impl(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn status_impl(&self) -> Result<TaskStatus, Box<dyn std::error::Error>> {
        let container = self
            .docker
            .inspect_container(self.container_name, None::<InspectContainerOptions>)
            .await?;
        ...
        let status = match container.state.and_then(|s| s.status) {
            ...
            None => {
                println!(
                    "Unable to retrieve status for container {}, assuming task is broken!",
                    self.container_name
                );
        ...
        println!(
            "The container status check returned the following: {}",
            status
        );
        ...
        match status {
            ...
            | ContainerStateStatusEnum::CREATED => Ok(TaskStatus::Stopped),
            ContainerStateStatusEnum::RUNNING => Ok(TaskStatus::Running),
            ContainerStateStatusEnum::REMOVING => Ok(TaskStatus::Cleaning),
            ContainerStateStatusEnum::DEAD | ContainerStateStatusEnum::EMPTY => {
                Ok(TaskStatus::Broken)
            }
            ContainerStateStatusEnum::RESTARTING => Ok(TaskStatus::Starting),
        }
    }

    pub async fn setup_api(&self) -> Result<(), Box<dyn std::error::Error>> {
        use_file(
            &Resources::CONTAINER_ACCESS_API.target.into(),
            Resources::CONTAINER_ACCESS_API.content,
            |api_path| {
                let mut permissions = fs::metadata(api_path)?.permissions();
        ...
        use_file(
            ...
            |modify_path| {
                let mut permissions = fs::metadata(modify_path)?.permissions();
                permissions.set_mode(0o755);
            ...
            |service_path| {
                ...
                if !restart_output.status.success() {
                    ...
                    if !start_output.status.success() {
                        return Err(format!(
                            "Failed to start container access API service: {}",
