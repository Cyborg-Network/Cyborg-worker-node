...
pub mod pallet {
    ...
    pub type TaskStatus<T: Config> =
        StorageMap<_, Twox64Concat, TaskId, TaskStatusType, OptionQuery>;
            ...
            match task_verification {
                Some(ref verification) => {
                    ...
                    if verification.executor.completed_hash == Some(completed_hash) {
                        ...
                        let block_number_encoded =
                            <frame_system::Pallet<T>>::block_number().encode();
                        task_verification_encoded.extend(block_number_encoded);

                        let random_index = (sp_io::hashing::blake2_256(&task_verification_encoded)
                            [0] as usize)
                            % workers.len();
                        let assigned_resolver: (T::AccountId, WorkerId) =
                            workers[random_index].0.clone();
                ...
                Some(ref verification) => {
                    ...
                    // TODO! Reward and Slash disputing completed_hash (require implement tokenomics)
                        ...
                        let random_index = (sp_io::hashing::blake2_256(&task_verification_encoded)
                            [0] as usize)
                            % workers.len();
                        let assigned_new_executor: (T::AccountId, WorkerId) =
                            workers[random_index].0.clone();
