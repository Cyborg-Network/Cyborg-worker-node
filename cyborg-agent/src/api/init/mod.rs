...
use crate::{crypto::encrypt_message, error_handling::ClientError};
...
impl Init {
    ...
    pub async fn return_init_message(
        ...
        let init_item = Init::get_init()
            .await
            .map_err(|e| ClientError::InitError(e.to_string()))?;

        let init_item =
            serde_json::to_string(&init_item).map_err(|e| ClientError::InitError(e.to_string()))?;
